...
function PRODUCT (id, category, name, description, produccer, price, count, discount) {
...
let Busket = {
		...
		try {
			...
			for (let val in Busket){
				if (typeof Busket[val] === 'object'){
					if (Busket[val].constructor.name === 'PRODUCT') {
						boo_IsProduct = true;
						break;
					}
				}
			...
			btn_busket.removeEventListener('click', Busket.open);
			btn_busket.addEventListener('click', Busket.close);
		} catch (e) {
			console.log(e);
		}
	},
	...
	close: function () {
		...
		btn_busket.removeEventListener('click', Busket.close);
		btn_busket.addEventListener('click', Busket.open);
	}
};

// создадим несколько позиций в корзине
Busket['pos1'] = new PRODUCT(1, 'Жвачка', 'Бубльгум', 'Жутко вкусная жвачка', 'Bubl-production', 200, 800, 0.05);
Busket.pos1.usercount = 5;
Busket['pos2'] = new PRODUCT(2, 'Жвачка', 'Дирол', 'Мятная жвачка', 'Dirol-production', 400, 1500, 0.1);
Busket.pos2.usercount = 3;
Busket['pos3'] = new PRODUCT(3, 'Жвачка', 'Ментос', 'Мятная конфета', 'Mentos-production', 800, 400, 0.25);
